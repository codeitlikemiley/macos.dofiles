...
# load cargo installed binaries
[ -f "$HOME/.cargo/env" ] && source $HOME/.cargo/env
# starship is loaded , and can be initialized
...
# Avoid entering passphrase everytime , when using ssh on git
[ -s ~/.ssh/id_codeitlikemiley ] && ssh-add --apple-use-keychain ~/.ssh/id_codeitlikemiley >/dev/null 2>&1

# Load All Aliases
[ -s "$HOME/.config/shell/aliasrc" ] && source $HOME/.config/shell/aliasrc
